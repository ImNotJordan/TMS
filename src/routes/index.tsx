  ...
  head: () => ({
    meta: [
      { title: "Dashboard — Logistics Software" },
      {
        name: "description",
        content:
          "Live operations command center: loads, bids, quotes, risk, revenue, and tracking.",
      },
    ],
  }),
...
export function DashboardPage() {
  ...
  const derived = React.useMemo(() => {
    ...
    const revenueDelta = prevMonth > 0 ? ((thisMonth - prevMonth) / prevMonth) * 100 : null;
  ...
  return (
    <div>
      <PageHeader
        ...
        description={t("Real-time view of loads, bids, quotes, carriers, and revenue.")}
        actions={
      ...
      <div className="space-y-6 px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <Card className="lg:col-span-2 border-border/70 shadow-sm">
            <CardHeader className="flex flex-row items-start justify-between gap-2 pb-2">
              <div>
                <CardTitle className="text-base">{t("Revenue vs Carrier Cost")}</CardTitle>
                <CardDescription>{t("Weekly booked revenue · last 8 weeks")}</CardDescription>
              </div>
          ...
          <Card className="border-border/70 shadow-sm">
            <CardHeader className="pb-2">
              <CardTitle className="text-base">{t("Live Shipments")}</CardTitle>
              <CardDescription>
                {isLoading
                  ? "Loading…"
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <Card className="lg:col-span-2 border-border/70 shadow-sm">
            <CardHeader className="flex flex-row items-center justify-between pb-3">
              <div>
                <CardTitle className="text-base">{t("Active Loads")}</CardTitle>
                <CardDescription>
                  {isLoading
              ...
              {loadError && (
                <div className="flex items-start gap-3 border-b border-destructive/30 bg-destructive/8 px-6 py-3 text-sm text-destructive">
                  ...
                  <div className="flex-1">
                    <div className="font-semibold">{t("Couldn't load loads")}</div>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="border-border/70">
                      ...
                      <TableHead className="pr-6 text-right">{t("Rate")}</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
          ...
          <div className="space-y-4">
            <Card className="border-border/70 shadow-sm">
              <CardHeader className="pb-2">
                <CardTitle className="text-base">{t("Alerts & Exceptions")}</CardTitle>
                <CardDescription>{t("Requires action")}</CardDescription>
              </CardHeader>
            ...
            <Card className="border-border/70 shadow-sm">
              <CardHeader className="pb-2">
                ...
                <CardDescription>{t("RFP due dates & quote pickups")}</CardDescription>
              </CardHeader>
              <CardContent className="space-y-2.5">
                {isLoading ? (
                  ...
                  derived.deadlines.map((d) => (
                    <div
                      key={d.label}
                      className="flex items-start justify-between gap-2 rounded-lg p-2 hover:bg-muted/40"
                    >
                      <div className="min-w-0">
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <Card className="lg:col-span-2 border-border/70 shadow-sm">
            <CardHeader className="pb-2">
              <CardTitle className="text-base">{t("Top Lanes by Volume")}</CardTitle>
              <CardDescription>{t("All loads · grouped by pickup → delivery")}</CardDescription>
            </CardHeader>
            <CardContent className="pt-2">
              {isLoading ? (
                <ChartSkeleton />
          ...
          <Card className="border-border/70 shadow-sm">
            <CardHeader className="pb-2">
              <CardTitle className="text-base">{t("Carrier Performance")}</CardTitle>
              <CardDescription>
                OTD% minus claims rate · top {Math.max(derived.carrierPerf.length, 1)}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {isLoading ? (
                ...
                <div className="rounded-lg border border-dashed border-border p-4 text-center text-sm text-muted-foreground">
                  {t("No scored carriers yet — add OTD% on carrier profiles.")}
                </div>
              ) : (
        ...
        <Card className="border-border/70 shadow-sm">
          <CardHeader className="pb-2">
            ...
            <CardDescription>
              {t("Latest changes across loads, quotes, RFPs & carriers")}
            </CardDescription>
          </CardHeader>
          <CardContent>
            {isLoading ? (
