...
import { Card, FieldShell, GridSection, SectionTitle, formatCurrency, formatTimestamp, generateCrmId } from "./crm-shared";
    ...
    <div className="space-y-6">
      <Card>
        <SectionTitle
          ...
          hint="Scans DAT by lane/equipment, calls brokers, verifies availability, negotiates within your margin floor"
          icon={Radar}
        />
        ...
        <GridSection cols={4}>
          <FieldShell label="Origin City">
            <Input value={originCity} onChange={(e) => setOriginCity(e.target.value)} placeholder="Dallas" />
          </FieldShell>
          <FieldShell label="Origin State">
            <Input value={originState} onChange={(e) => setOriginState(e.target.value)} placeholder="TX" />
          </FieldShell>
          <FieldShell label="Dest City">
            <Input value={destCity} onChange={(e) => setDestCity(e.target.value)} placeholder="Atlanta" />
          </FieldShell>
          <FieldShell label="Dest State">
            <Input value={destState} onChange={(e) => setDestState(e.target.value)} placeholder="GA" />
          </FieldShell>
        </GridSection>
        <GridSection cols={4} className="mt-4">
          ...
          <FieldShell label="Target $/mile">
            <Input type="number" step="0.05" value={targetRatePerMile} onChange={(e) => setTargetRatePerMile(e.target.value)} />
          </FieldShell>
        ...
        <div className="mt-4 flex items-center justify-between">
          <FieldShell label="Max results" className="w-32">
            <Input type="number" value={maxResults} onChange={(e) => setMaxResults(e.target.value)} />
          </FieldShell>
          <Button type="button" className="gap-1.5" disabled={scanning} onClick={() => void runScan()}>
          ...
          <Card key={run.runId} className={idx === 0 ? "" : "opacity-90"}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-sm font-medium text-foreground">
                {run.originCity ?? "—"}, {run.originState ?? ""} → {run.destCity ?? "—"}, {run.destState ?? ""}
              ...
              <div className="flex items-center gap-3 text-xs text-muted-foreground">
                ...
                >
                  <Radar className="h-3 w-3" /> DAT search
                </a>
              </div>
            </div>
            <div className="mt-3 space-y-2">
              {run.matches.map((match) => {
                ...
                return (
                  <div key={match.matchId} className="rounded-lg border border-border/60 bg-card/60 p-3">
                    ...
                    <div className="mt-2 flex items-center justify-between gap-2">
                      ...
                      <div className="flex gap-2">
                        ...
                        >
                          ...
                          {match.callOutcome && match.callOutcome !== "pending" ? "Call again" : "Call broker"}
                        </Button>
                        {negotiated && (
                          <Button
                            type="button"
                    ...
                    {match.guardrailReason && (
                      <div
                        className={cn(
                          ...
                          match.guardrailAllowed ? "bg-success/12 text-success" : "bg-destructive/12 text-destructive",
                        )}
                      >
